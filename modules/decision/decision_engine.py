...
class DecisionEngine:
    ...
    def analyze(self, liquidity_data, svd_data, market_structure, ta_data, current_price=None, htf_context=None, htf_liquidity=None, trading_entry=None):
        """
        ...
        Args:
            ...
            htf_context: контекст старших таймфреймов
            htf_liquidity: ликвидность на старших таймфреймах
            trading_entry: торговый сигнал от EntryStrategy (опционально)
        ...
        signals = {
            ...
            "trading_entry": trading_entry or {}
        }
    ...
    def _determine_direction(self, signals):
        ...
        # Торговый сигнал от EntryStrategy (вес 2.0 - высокий приоритет для коридоров)
        trading_entry = signals.get("trading_entry", {})
        entry_signal = trading_entry.get("entry_signal", "WAIT")
        entry_confidence = trading_entry.get("entry_confidence", 0.0)
        if entry_signal != "WAIT" and entry_confidence > 0.5:
            # Торговый сигнал имеет высокий вес, если confidence > 0.5
            if entry_signal == "BUY":
                votes["BUY"] += 2.0 * entry_confidence  # Максимум 2.0 при confidence=1.0
            elif entry_signal == "SELL":
                votes["SELL"] += 2.0 * entry_confidence
    ...
    def _calculate_confidence(self, signals):
        ...
        # БОНУС: Торговый сигнал от EntryStrategy (коридор + накопление)
        trading_entry = signals.get("trading_entry", {})
        entry_signal = trading_entry.get("entry_signal", "WAIT")
        entry_confidence = trading_entry.get("entry_confidence", 0.0)
        ...
        if entry_signal != "WAIT" and entry_confidence > 0.5:
            # Бонус зависит от confidence торгового сигнала и R/R
            trading_bonus = entry_confidence * 2.0  # Максимум +2.0 при confidence=1.0
            
            # Дополнительный бонус за хороший R/R (если R/R > 1.5)
            if risk_reward > 1.5:
                trading_bonus += 0.5
            elif risk_reward > 1.0:
                trading_bonus += 0.3
            
            # Бонус применяется только если торговый сигнал совпадает с финальным направлением
            if entry_signal == signals.get("signal"):
                final_confidence += trading_bonus
                import logging
                logger = logging.getLogger(__name__)
                logger.info(f"📈 Бонус от торговой системы: +{trading_bonus:.2f} "
                           f"(entry_confidence: {entry_confidence:.2f}, R/R: {risk_reward:.2f})")
