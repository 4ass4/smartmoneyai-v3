...
def apply_risk_filters(signals, confidence):
    ...
    if signal_type == "BUY" and svd_intent == "distributing":
        return {
            "allowed": False,
            "reason": "🚫 ОПАСНО: BUY когда киты распределяют (CVD отрицательный). Риск слива!"
        }
    ...
    if signal_type == "SELL" and svd_intent == "accumulating":
        return {
            "allowed": False,
            "reason": "🚫 ОПАСНО: SELL когда киты накапливают (CVD положительный). Риск слива!"
        }
    
    # Дополнительные конфликты (менее критичные, но учитываем)
    minor_conflicts = 0
    ...
    # Блокируем если есть минорные конфликты (дополнительная защита)
    if minor_conflicts >= 1:
        return {
            "allowed": False,
            "reason": f"Противоречия в направлении ликвидности (minor conflicts: {minor_conflicts})"
        }
